...
__all__ = ["JSSAddonSourceFinder"]



class JSSAddonSourceFinder(Processor):
    """Finds the root arubdesu-JSS-addon-foo folder from the expanded autopkg zip archive"""
    input_variables = {
        "input_path": {
            "required": True,
    ...
    def main(self):
        ...
        try:
            autopkg_dir = self.find_match(root_dir, 'arubdesu-jss-autopkg-addon-*')
            self.env["jssaddon_path"] = os.path.join(root_dir, autopkg_dir)
            self.output("Found %s" % self.env["jssaddon_path"])
